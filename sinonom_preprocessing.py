...
class SinoNomPreprocessor:
    ...
    def split_sents(self, text) -> List:
        ...
        #     if not sent:
        #         continue
        #     # Nếu câu ngắn vừa phải thì giữ nguyên
        #     if len(sent) <= max_len:
        #         sentences.append(sent)
        #     else:
        #         # Nếu quá dài, chia tiếp theo dấu phẩy hoặc khoảng trắng
    ...
    def norm_and_split(self, sect_text: str) -> List:
        # paragraph splitting
...
def sinonom_preprocessing(section_text: str) -> Dict:
    # paragraph splitting
    paragraph_section_text = [s for s in section_text.split(PARAGRAPH_BREAK) if s.strip()]
    
    # normalization
    norm_para_section_text = []
    for p in paragraph_section_text:
        norm_para_section_text.append(normalize_chinese_text(p))

    # sentence splitting
    sentence_para_section_text = []
    for p in norm_para_section_text:
        sentence_para_section_text.append(split_chinese_sentences(p))
...
def split_chinese_sentences(text) -> List:
    """
    Tách văn bản tiếng Hán thành từng câu hoàn chỉnh.
